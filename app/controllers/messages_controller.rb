    ...
    begin
      ...
      if User.exists?(name: params[:recipientname]) then
        if decrypt_digest == digest then
          ...
          respond_to do |format|
            format.json { render json: @success = '{"status":"' + params[:timestamp].to_s + params[:recipientname].to_s + params[:name].to_s + params[:cipher].to_s + params[:iv].to_s + params[:key_recipient_enc].to_s + params[:sig_recipient].to_s + 'Unser Digest:' + digest.to_s + 'Marians Digest' + params[:sig_service].to_s  +  '"}'}
          end
        end
      else
        respond_to do |format|
